...
class PCIProtocol(CBusProtocol):
        ...
        if isinstance(p, SpecialServerPacket):
            ...
            for cal in p:
                if (isinstance(cal, ExtendedCAL)
                        and cal.child_application == Application.LIGHTING
                        and isinstance(cal.report, LevelStatusReport)):
                    self.on_lighting_group_status(
                        cal.block_start, cal.report)
                else:
                    logger.debug(f'hcp: unhandled PTP CAL: {cal!r}')
        else:
    ...
    def on_lighting_group_status(self, block_start: int,
                                 report: LevelStatusReport):
        """
        Event called when a level status report is received.

        :param block_start: First group address represented by the report.
        :param report: Brightness levels, with ``None`` for missing groups.
        """
        logger.debug(
            f'recv: light status: from {block_start} for {len(report)} groups')
    ...
    def request_lighting_status(self, group_addr: Optional[int] = None):
        """
        Requests brightness levels for one group or all lighting groups.

        C-Bus status requests address a 32-group block, so a request for one
        group returns the block containing it.
        """
        if group_addr is None:
            group_addresses = range(0, 0x100, 0x20)
        else:
            group_addr = int(group_addr)
            if group_addr < 0 or group_addr > 0xff:
                raise ValueError(
                    f'group_addr must be in range 0..255 (got {group_addr})')
            group_addresses = [(group_addr // 0x20) * 0x20]

        confirmations = []
        for block_start in group_addresses:
            packet = PointToMultipointPacket(sals=StatusRequestSAL(
                level_request=True,
                group_address=block_start,
                child_application=Application.LIGHTING))
            confirmations.append(self._send(packet))
