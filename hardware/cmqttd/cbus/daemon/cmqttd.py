...
def get_topic(group_addr: int) -> Text:
    """Gets the status request topic for a group address."""
    return _LIGHT_TOPIC_PREFIX + str(group_addr) + _TOPIC_GET_SUFFIX
...
class CBusHandler(PCIProtocol):
    ...
    def on_lighting_group_status(self, block_start, report):
        if not self.mqtt_api:
            return
        self.mqtt_api.lighting_group_status(block_start, report)
...
class MqttClient(mqtt.Client):
    ...
    def on_message(self, client, userdata: CBusHandler, msg: mqtt.MQTTMessage):
        ...
        if msg.topic.endswith(_TOPIC_GET_SUFFIX):
            userdata.request_lighting_status(ga)
            return
    ...
    def lighting_group_status(self, block_start: int,
                              report: LevelStatusReport):
        """Publishes brightness levels returned by a C-Bus status request."""
        for offset, level in enumerate(report):
            if level is None:
                continue
            group_addr = block_start + offset
            self.publish(state_topic(group_addr), {
